...
const ProfilePage = () => {
  ...
  // profile header state (was missing)
  const [profilePic, setProfilePic] = useState(
    () => localStorage.getItem("profilePic") || "https://i.ytimg.com/vi/rvX8cS-v2XM/maxresdefault.jpg"
  );
  const [editingPic, setEditingPic] = useState(false); // false | "idle" | "readyToSave"
  const [bio, setBio] = useState(() => localStorage.getItem("bio") || "No bio yet. Add one to tell others about yourself!");
  ...
  // Load user's posts from storage; if no posts, fall back to dogs once
  useEffect(() => {
    const user = getCurrentUser();
    const username = user?.fullName ? user.fullName.split(" ")[0].toLowerCase() : "me";
    const posts = getPostsByUsername(username);
  ...
  useEffect(() => {
    ...
    const username = user?.fullName ? user.fullName.split(" ")[0].toLowerCase() : "me";
    const posts = getPostsByUsername(username);
    if (posts?.length) setPhotos(posts);
  ...
  useEffect(() => {
    ...
    // Depend on selectedPost.id for stability
  ...
  return (
    <>
      <Container maxWidth="md" sx={{ textAlign: "center", mt: 10, mb: 4 }}>
        {/* Profile header (outside the grid) */}
        <Box sx={{ textAlign: "center" }}>
          <Avatar
            alt="User Avatar"
            src={profilePic}
            ...
            onClick={() => {
              if (!editingPic || editingPic === false) setEditingPic("idle");
            }}
          />
          ...
          {editingPic === "idle" && (
            <Box sx={{ display: "flex", justifyContent: "center", gap: 1, mt: 1 }}>
              ...
              <Button size="small" onClick={() => setEditingPic(false)}>
                Cancel
              </Button>
            </Box>
          )}
        ...
        >
          {editingBio ? (
            <Box>
              ...
              />
              <Box sx={{ mt: 1 }}>
                <Button variant="contained" size="small" onClick={handleBioSave}>
                  Save
                </Button>
                <Button size="small" sx={{ ml: 1 }} onClick={() => setEditingBio(false)}>
                  Cancel
                </Button>
              </Box>
            </Box>
